	...
	for (int jjj = 0; jjj < 64; jjj++) {
		for (ask1; ask1 < 73; ask1++) {
			for (ask2; ask2 < 73; ask2++) {
				...
				count++;
				cout << "최종 quantity 값은 " << qun[jjj] << endl;
				temp123 = 0;
			}
			ask2 = 65;
		}
	}
	cout << "가장 큰 quantity 값은 " << temp_max << " 입니다." << endl;
	cout << "위치는 " << save_point + 1 << " 입니다." << endl;
	row_save = (save_point / 8) + 1;
	col_save = (save_point + 1) % 8;
	qun[save_point] = temp_max;
	cout << "행 " << row_save << " 열 " << col_save << endl;
